	...
	case TIMER_EVENT:
		{
			...
			{
				double* result = NULL;
				result = fn_netsim_labview_run();
				fprintf(stderr, "\nValue returned from LABVIEW: %lf\n", *result);
				pstruEventDetails->dEventTime += 10.0 * SECOND;
				pstruEventDetails->nEventType = TIMER_EVENT;
				pstruEventDetails->nProtocolId = MAC_PROTOCOL_IEEE802_15_4;
				pstruEventDetails->nSubEventType = LABVIEW_EVENT;
				fnpAddEvent(pstruEventDetails);
			}
			break;
			//NetSim Labview Interfacing
	...
	char *pszWritePath_Formal,int nVersion_Type,void **fnPointer)
{
	...
	//NetSim Labview Interfacing
	fprintf(stderr, "\nInitializing Labview");
	fprintf(stderr, "\nPress any key to continue...");
	getch();
	fn_netsim_labview_init();
...
_declspec(dllexport) int fn_NetSim_Zigbee_Finish()
{
	...
	//NetSim Labview Interfacing
	fprintf(stderr, "\nQuitting LABVIEW Automation Server\n");
	fprintf(stderr, "\nPress any key to continue..\n");
	getch();
	fn_netsim_labview_finish();
	//NetSim Labview Interfacing
	return 0;
}	
